...
# SONIC RINGWORLDS
Sonic Ringworlds is a project built off of Sonic Z-Treme
the project is made to see whether Z-Treme can hold up a full fledged game and whether it has the capability of being optimized any more

SONIC RINGWORLDS v0.0004
...
# To play the game [Ringworlds]:
simply boot up a batch file that has the name of an emulator
if there is no file to play with, compile and you'll be on your way.

# To compile : 
Just run compile.bat to compile the code. I use Code::Blocks, but even notepad would work.

# Note on this code [XL2]: 
...
# A new note on the code [Birb64]:
the code still isn't as pretty.
I don't know if there is any better ways to program this stuff. Props to XL2 for helping us with Z-Treme as we move along.
I myself am not the most experienced programmer. Just looking at XL2's code taught me way more programming tricks that I never thought existed.
I'll do my best to optimize, shorten, and sharpen the code. This doesn't mean that the code will be much better than it already is
Ringworlds is planned to have a lot more content so it'll take longer than Z-Treme did.
That isn't to say that Z-Treme is not as good, in fact it's the foundation of Ringworlds. It's fun to use and I highly recommend to use it if you're not the most experienced programmer.

# Why Sonic X-Treme?[XL2]
...
# A thank you
Had it not been for you choosing sonic X=Treme as a recreation, Ringworlds would have never had much of a good engine to work with. 
We were originally working with Unity and it's not the best to use if you're working on a game with complicated angles. 
Though this means I'm mostly gonna be using someone else's code(something I'd usually not like as much to do) I still trust that this code is way better than what I could do in Unity
thank you XL2
...
# The surgery[Birb64]
"I refuse to play this unless you put sprites!" 
this won't be fixed, however it will be compensated by a simple answer: project condor. Since Ringworlds is a mashup style of game, it mashes all the sonic X-Treme protos, including project condor. So that will be the answer to that issue
"Port Sonic Mania instead! Do I have to do it myself?"
There isn't a reason to port sonic mania to saturn, however if you did want to port it DIY, just use JoEngine. As I've heard, JoEngine is used for 2D games. You could use the code from this project/sonic Z-Treme and have the physics work according to sonic Z-Treme
"Where is the fisheye lens?"
no.
"Can you make game XYZ instead?"
I don't have much to say here, I don't know what this means. So, no?
"There is a reason Sega cancelled Sonic X-Treme!"
don't worry, me and the Ringworlds team have figured out what made sonic X-Treme so poor and have found a multitude of ways to fix every problem whilst also keeping the functionalities of those fixes(one way we did this is we took out the fisheye lens and replaced it with a nice 45 degree angle change in camera)
"Mario 64/Crash is much better!"
Z-Treme wasn't fully completed, as it was a simple, yet effective and ambitious, project. Ringworlds is planned to be more complete and with tons more to do.
"It's so blocky! LOL!"
I can't say much to this, as sonic Ringworlds is still a bit blocky, however, it's blocky in a way that makes it more akin to super mario 3D world in a way

No matter what you say, I think this project will go nicely if the Ringworlds team can keep up

# Note on this build [XL2]: 
...
# A new note on this build [Birb64]:
this version isn't as rushed as Z-Treme. We in the Ringworlds team are taking our time very leisurely, as we don't want to have this project effect our lives too negatively.

# About the map compiler [XL2]: 
It truly sucks. 
...
# About the map compiler 2 [Birb64]:
XL2 didn't give us much info on the map compiler
He did say that he would lend us a copy of it once he got it working enough.
We here down at Ringworlds can't wait to see the map compiler, mapping has been a huge thing since we've started in unity.

# A reqiuem to the unity ringworlds
The unity version of ringworlds was all over the place. I was the only one programming and I wanted the physics to be perfect. 
It took us 2 months before we had normal wall collision. Even then, the wall collision was terrible and not finished. 
The thing that stands out from Z-Treme is that it had a map maker and planned modding capabilities.
We had a map maker which saved in .xtra files(since ringworlds was originally xtraordinary) the map maker's name was SS.PC.
Fully capable of making blocks, and 45 degree slopes with differently textured sides.
Everyone, including I, enjoyed it quite a lot.
It was fun, simple, and effective.
I'm hoping I can get a hold of XL2's map maker source code. 
So that I might be able to make a mod for blender that would add icons and such to the game that would export to the map file.
